...
const CakeBenefitsCard: React.FC<React.PropsWithChildren<CakeBenefitsCardProps>> = ({ onDismiss }) => {
  ...
  const {
    ...
    <>
      <Text>
        {t(`iCAKE allows you to participate in the IFO public sales and commit up to %iCake% amount of KFC.`, {
  ...
  const {
    ...
    <>
      <Text>{t(`bCAKE allows you to boost your yield in Kaspa Finance Farms by up to 2x.`)}</Text>
  ...
  const {
    ...
    <>
      <Text>
        {t(`vCAKE boosts your voting power to %totalScore% in the Kaspa Finance voting governance.`, {
        ...
        <>
          ...
          <Message mt="8px" mb="16px" variant="warning">
            <MessageText maxWidth="200px">
              {t(
                'Lock KFC to enjoy the benefits of farm yield boosting, participating in IFOs, voting power boosts, and so much more!',
              )}{' '}
              <NextLinkFromReactRouter
