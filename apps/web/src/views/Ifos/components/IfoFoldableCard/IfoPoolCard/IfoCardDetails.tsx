...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  ...
  const basicTooltipContent =
    ifo.version >= 3.1
      ...
      : t(
        'For the basic sale, Max KFC entry is capped by minimum between your average KFC balance in the iCAKE, or the pool’s hard cap. To increase the max entry, Stake more KFC into the iCAKE',
      )
  ...
  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      ...
      t(
        'For the unlimited sale, Max KFC entry is capped by your average KFC balance in the iCake. To increase the max entry, Stake more KFC into the iCake',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. KFC entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {raisingTokenToBurn && <FooterEntry label={t('KFC to burn:')} value={raisingTokenToBurn} />}
          <FooterEntry
