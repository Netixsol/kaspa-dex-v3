...
export const multicallAddresses = {
  ...
  [ChainId.KASPLEX_MAINNET]: '0xcA11bde05977b3631167028862bE2a173976CA11',
}
...
export function createMulticall<TProvider extends Provider>(
  ...
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.KASPLEX_TESTNET) => {
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.KASPLEX_TESTNET, options, provider: _provider }) => {
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.KASPLEX_TESTNET, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
